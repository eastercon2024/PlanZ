...
SELECT
    ...
    WHERE
        POS.sessionid IN (
            SELECT sessionid FROM ParticipantOnSession WHERE badgeid='$partbadgeid'
			)
...
$query .= "WHERE C.badgeid='$partbadgeid'";
...
$query .= "WHERE badgeid='$partbadgeid'";
if (!$result = mysqli_query_with_error_handling($query)) {
    exit(); // Should have exited already
}
$row = mysqli_fetch_array($result, MYSQLI_NUM);
...
RenderXSLT('my_schedule.xsl', array( "badgeid" => $partbadgeid, "allowConfirmation" => $allowConfirmation ), $resultXML);
?>
    </div>
</div>
</div>
