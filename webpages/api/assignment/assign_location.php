...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && $authentication->isProgrammingStaff()) {
        ...
        if (array_key_exists('sessionId', $json) && array_key_exists('location', $json)
            ...
            if ($assignment != null) {
                $assignment->location = $json["location"];
                ParticipantAssignment::updateLocation($db, $assignment, $authentication);
