...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="/">
        <div class="container">
            ...
            <form method="POST" action="CreateAccountSubmit.php" class="well form-horizontal">
                <div class="card mt-3 mb-3">
                    <div class="card-body">
                        <input type="hidden" id="control" name="control" value="{$control}" />
                        <input type="hidden" id="controliv" name="controliv" value="{$controliv}" />
                        <p>To create your account, you must first choose a password.</p>
                        <p>Enter a password for <xsl:value-of select="$user_name" />, badgeid: <xsl:value-of select="$badgeid" /></p>
                ...
                <p>
                    After setting your password, you will be taken to the login page.
                </p>
            </form>
        </div>
    </xsl:template>
